...
![](https://cdn.hashnode.com/res/hashnode/image/upload/v1696609040735/891eb351-644d-4d9d-ae5d-4f522863a890.png align="center")

設為絕對定位的元素，會依據「父層中距離自己最近有定位的元素」定位，也就是除了 `static` 以外的定位屬性都可以作為定位的對象。要注意，它是找離自己最近的，當網頁規模很多層時容易出錯，所以要規劃好 HTML 中元素之間的關係。

如果都沒有的話，它會依據瀏覽器視窗（`viewport`）大小定位。
...
> 如果 fixed 的項目上有 `filter` 或 `backdrop-filter` 屬性，就不會依據「瀏覽器的視窗大小」進行定位，而是依據「HTML 的 body」定位。
> 
> 解法：改變 `filter` 的元素，或是改使用 `sticky` 做到類似效果。
> 
> 延伸閱讀：[filter与fixed冲突的原因及解决方案 - 掘金](https://juejin.cn/post/6844904117974859783)
...
Edge 的開發者偵錯工具，有酷炫有趣的 3D View 可以參考網頁中的前後關係：
