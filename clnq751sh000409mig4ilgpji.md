...
這種方式建議**只使用在單行的時候**：如果使用在多行的情況，它會在每一行的尾巴都添加「…」，但這並不是我們期望的效果，我們只需要最後一行有添加「…」就好了 。
...
`<pre>` 是 HTML 中用來顯示程式碼片段的標籤，預設具有「依據內容（`\r` `\n`）自動斷行，但是文字超出容器時他不會斷行，並且所有空格都會如實顯示」的特性，這段對應的 CSS 屬性，便是 `white-space: pre;`。
